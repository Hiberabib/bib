UNIX BUILD NOTES
====================
Some notes on how to build HIBERA Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile HIBERA Core and the dependencies,
...
This will build hibera-qt as well, if the dependencies are met.
...
memory available when compiling HIBERA Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~hibera/+archive/hibera).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:hibera/hibera
    sudo apt-get update
...
To build HIBERA Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build hibera-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a hibera-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip hiberad" to strip the debug
...
Security
--------
To help make your HIBERA Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, HIBERA Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, HIBERA Core may be compiled in
