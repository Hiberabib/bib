...
*Setup instructions for a gitian build of HIBERA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the HIBERA
...
to the HIBERA GitHub release page.
...
- [Building HIBERA Core](#building-hibera-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/hibera-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/hibera-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/hibera-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/hibera-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/hibera-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/hibera-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/hibera-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@hibera.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/hibera-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/hibera-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/hibera-osx-*-build.assert
```
