...
#if defined(NDEBUG)
#error "HIBERA cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uBIB) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("hibera-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        const bool hasBIBInputs = !bibInputs.empty();
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // HIBERA: We use certain sporks during IBD, so check to see if they are
