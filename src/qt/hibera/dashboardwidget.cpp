...
#include "qt/hibera/dashboardwidget.h"
#include "qt/hibera/forms/ui_dashboardwidget.h"
#include "qt/hibera/sendconfirmdialog.h"
#include "qt/hibera/txrow.h"
...
DashboardWidget::DashboardWidget(HIBERAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BIB earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 BIB");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &HIBERAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &HIBERAGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &HIBERAGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("BIB")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair BIB, zBIB
