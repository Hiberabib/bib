...
#include "qt/hibera/forms/ui_snackbar.h"
#include "qt/hibera/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(HIBERAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &HIBERAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
