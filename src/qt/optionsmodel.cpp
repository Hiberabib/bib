...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BIB);
    ...
    if (settings.contains("nAnonymizeHiberaAmount") || reset)
        SoftSetArg("-anonymizehiberaamount", settings.value("nAnonymizeHiberaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hibera.cpp during shotdown to also remove the window positions
