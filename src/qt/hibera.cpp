...
#include "config/hibera-config.h"
#endif
...
#include "qt/hibera/splash.h"
#include "qt/hibera/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("hibera-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in hibera.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in hibera.qrc)
...
/** Class encapsulating HIBERA Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (HIBERAGUI)
...
{
    window = new HIBERAGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &HIBERAGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &HIBERAGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(HIBERAGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(HIBERAGUI::DEFAULT_WALLET);
        ...
        // HIBERA: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HIBERAGUI::handlePaymentRequest);
        connect(window, &HIBERAGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. HIBERA can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(hibera_locale);
    Q_INIT_RESOURCE(hibera);
    ...
    /// 6. Determine availability of data directory and parse hibera.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("HIBERA"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("HIBERA"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // hibera: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("HIBERA didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
